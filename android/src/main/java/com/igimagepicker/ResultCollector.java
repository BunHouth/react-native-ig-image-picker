 ...
 * https://github.com/ivpusic/react-native-ig-image-picker/blob/master/android/src/main/java/com/reactnative/ivpusic/imagepicker/ResultCollector.java
 */
...
class ResultCollector {
    ...
    synchronized private boolean isRequestValid() {
        if (resultSent) {
            Log.w("ig-image-picker", "Skipping result, already sent...");
            return false;
        }

        if (promise == null) {
            Log.w("ig-image-picker", "Trying to notify success but promise is not set");
            return false;
        }
    ...
    synchronized void notifyProblem(String code, String message) {
        ...
        Log.e("ig-image-picker", "Promise rejected. " + message);
    ...
    synchronized void notifyProblem(String code, Throwable throwable) {
        ...
        Log.e("ig-image-picker", "Promise rejected. " + throwable.getMessage());
